...
class FakeJobDetectionModel:
    ...
    def train(self, train_texts: List[str], train_labels: List[int], 
        ...
        """Train the BERT model on fake job posting data"""
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, len(train_texts), batch_size):
                ...
                batch_labels_tensor = torch.tensor(batch_labels, dtype=torch.float).to(self.device).view(-1, 1)
                ...
                # Stack predictions into a batch tensor
                batch_predictions = torch.cat(predictions, dim=0)
                
                # Calculate loss
                loss = loss_fn(batch_predictions, batch_labels_tensor)
                ...
                pred_labels = (batch_predictions > 0.5).int().squeeze()
                correct_predictions += (pred_labels == batch_labels_tensor.squeeze()).sum().item()
            ...
            if val_texts and val_labels:
                self.evaluate(val_texts, val_labels, batch_size)
        ...
        # Save the trained model
        self.save_model("fake_job_bert_model.pt")
        print("Training completed and model saved.")
    
    def evaluate(self, texts: List[str], labels: List[int], batch_size: int = 8):
